                ...
                if (z == 500) {
                  if (-1 == this.className.indexOf(" toggled-on")) {
                    ...
                    for (var i = 0; i < toggleClass.length; i++) {
                      toggleClass[i].className = toggleClass[
                        i
                      ].className.replace(" toggled-on", "");
                    }
                  }
                  if (-1 != this.className.indexOf(" toggled-on")) {
                    var cssClass =
                      document.getElementsByClassName("toggled-on");
                    for (var i = 0; i < cssClass.length; i++) {
                      cssClass[i].className = cssClass[i].className.replace(
                        " toggled-on",
                        ""
                      );
                    }
        ...
        parent.onclick = function (e) {
          ...
          if (y != "none") {
            ...
            if (-1 != childname[0].className.indexOf("vtoggled-on")) {
              childname[0].className = childname[0].className.replace(
                " vtoggled-on",
                ""
              );
            } else {
